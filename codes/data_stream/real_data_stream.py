import sys
sys.path.append('/data/david/project/HumLa-fse2023-main')
...
def set_test_stream(project_name):
    ...
    df = pd.read_csv(dir_load_data + project_name + "_final.csv")
    commit_test_stream = df.iloc[:, -2:]
    df = df.iloc[:,:-2]
    data_test_stream = DataStream(df, target_idx= -2)
    # data_test_stream = FileStream(df, target_idx=-2)
    # data_test_stream = FileStream(dir_load_data + project_name + "_modified.csv", target_idx=-4)
    # see skmultiflow.data_stream.file_stream.FileStream for how to use the data_test_stream
    return data_test_stream, commit_test_stream
