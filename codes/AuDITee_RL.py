...
from rl_agent.selector import RLTestSelector
from rl_agent.config import config
...
result_dir = "results_extend"

def run_AuDITee(project_id=3, seeds=range(10), selector=5, verbose_int=0, tau=0.001, is_plot=False):
    # prepare
    ...
    test_stream, commit_stream, count_in_t_windows = set_test_stream(project_name)
    ...
    for ss, seed in enumerate(seeds):
        ...
        test_result = pd.read_csv(
            f'../dataset/data/{project_name}_testing_results_with_extend/{project_name}_combined_seed{seed}.csv')
        test_result_ = {}
        for tr in range(len(test_result)):
            test_result_[test_result.iloc[tr][0]] = test_result.iloc[tr]['status']

        if selector == -1:
            file_path = f'../dataset/data/{result_dir}/{project_name}/ODaSC/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            ...
            writer.writerow(['commit_id', 'predict_prob_1', 'SDP_predict', 'commit_guru_result', 'test_result'])  # 写入CSV头部
        elif selector == 0:
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S1/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            ...
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S2/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            ...
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S3/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            ...
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S4/tau{tau}/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            ...
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S5/tau{tau}/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            ...
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S6/seed{seed}/rst.csv'
            # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            commit_result = open(file_path, 'w', newline='', encoding='utf-8')
            writer = csv.writer(commit_result)
            rl_selector = RLTestSelector(config,None)
            writer.writerow(
                ['commit_id', 'predict_prob_1', 'predict_prob_0', 'SDP_predict', 'commit_guru_result', 'test_result',
                    'action'])  # 写入CSV头部
        elif selector == 6:
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S7/seed{seed}/rst.csv'
             # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
            commit_result = open(file_path, 'w', newline='', encoding='utf-8')
            writer = csv.writer(commit_result)
            # y_ptrn_predict = classifier.predict_proba(X_ptrn)[:,0]
            # y_ptrn_predict = y_ptrn_predict.reshape(-1, 1)
            # label_data = zip(np.hstack((y_ptrn_predict,X_ptrn)), y_ptrn)
            rl_selector = RLTestSelector(config,None)
            writer.writerow(
                ['commit_id', 'predict_prob_1', 'predict_prob_0', 'SDP_predict', 'commit_guru_result', 'test_result',
                    'action'])  # 写入CSV头部
        elif selector == 7:
            file_path = f'../dataset/data/{result_dir}/{project_name}/AuDITee_S8/seed{seed}/rst.csv'
             # 确保目录存在，如果不存在则创建
            directory = os.path.dirname(file_path)
            if not os.path.exists(directory):
                os.makedirs(directory)
        ...
        for tt in range(nb_test_act):
            ...
            if selector == -1:
                ...
                writer.writerow([commit_id, y_pre_prob_[1], SDP_predict, int(test_1data[0][-1]), test_label])
            elif selector == 0:
                SDP_predict = 0
                ...
                if y_pre_prob_[1] >= 0.5 or random_variable <= y_pre_prob_[1]:
                    # if test_label >= 0:
                    if test_label == 1:
                        sota_X, sota_churn, sota_time, sota_y_tru = \
                            test_X, np.array([test_churn]), np.array([test_time[tt]]), np.array([test_y_tru[tt]])
                ...
                count_in_t_window = count_in_t_windows[test_step]
                score = y_pre_prob_[1]
                state = rl_selector.build_state(test_X, score)
                action = rl_selector.select_action(state)
                if action == 1:
                    if test_label == 1:
                        sota_X, sota_churn, sota_time, sota_y_tru = \
                            test_X, np.array([test_churn]), np.array([test_time[tt]]), np.array([test_y_tru[tt]])
                    ...
                    result = test_label
                    if selector == 5:
                        reward = rl_selector.evaluate_test_result(result)
                    else:
                        reward = rl_selector.evaluate_test_result_res(result)
                    rl_selector.store(state, action, reward)
                else:
                    y_true = np.array([test_y_tru[tt]])
                    reward = rl_selector.defer_feedback(test_X, score, y_true, count_in_t_window)
                    rl_selector.store(state, action, reward, done=False)
                
                rl_selector.learn()
                rl_selector.update_delayed_feedback()
                SDP_predict = 1 if y_pre_prob_[1] > 0.5 else 0

                writer.writerow(
                    [commit_id, y_pre_prob_[1], y_pre_prob_[0], SDP_predict, int(test_1data[0][-1]), test_label, action])
                ...
                rl_selector.learn()
                rl_selector.update_delayed_feedback_1()
                SDP_predict = 1 if y_pre_prob_[1] > 0.5 else 0
...
def change_s6_threshold(test_result, threshold, p1, p0):
    if test_result == 1:
        threshold -= p1
    # test 0
    elif test_result == 0:
...
if __name__ == "__main__":
    for selector in range(-1,1):
        for project in range(0,1):
            # selector = 4
            # project = 0
