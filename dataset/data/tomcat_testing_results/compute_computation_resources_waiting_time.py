...
for i in range(1,4):
    ...
    # 输出结果
    print(f"tomcat_sdp_commit_seed{i}.csv中总的数据量占tomcat_test_pre_seed{i}.csv总的数据量的比例为: {len(sdp_commit_ids) / len(pre_commit_ids):.2%}")
    print(f"tomcat_sdp_commit_seed{i}.csv中commit_id在diff_files.txt中对应的数组元素数量总和为: {sdp_total_array_length}")
