...
with open('diff_files.txt', 'r') as file:
    for line in file:
        ...
        commit_to_array_length[string_part] = len(elements) if len(elements) < 10 else 10
...
for i in range(1,4):
    ...
    df_sdp = pd.read_csv(f'broadleaf_sdp_test_commit_seed{i}.csv')
    ...
    # 输出结果
    print(f"broadleaf_sdp_test_commit_seed{i}.csv中总的数据量占broadleaf_test_pre_seed{i}.csv总的数据量的比例为: {len(sdp_commit_ids) / len(pre_commit_ids):.2%}")
    print(f"broadleaf_sdp_test_commit_seed{i}.csv中commit_id在diff_files.txt中对应的数组元素数量总和为: {sdp_total_array_length}")
    print(f"broadleaf_test_pre_seed{i}.csv中commit_id在diff_files.txt中对应的数组元素数量总和为: {pre_total_array_length}")
